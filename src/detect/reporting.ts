...
function createComponentRow(component: IComponentReport): string {
  ...
  const componentLicenses = component.licenses.map(license => `${license.violatesPolicy ? ':x: &nbsp; ' : ''}[${a}](${a})`).join('<br/>')
  console.log('componentLicenses::' + componentLicenses)
  const vulnerabilities = component.vulnerabilities.map(vulnerability => `${vulnerability.violatesPolicy ? ':x: &nbsp; ' : ''}[${a.href}](${vulnerability.href})${vulnerability.cvssScore && vulnerability.severity ? ` ${vulnerability.severity}: CVSS ${vulnerability.cvssScore}` : ''}`).join('<br/>')
  console.log('vulnerabilities::' + componentLicenses)
